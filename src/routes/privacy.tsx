  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">{t('最後更新：{date}', { date: t(LAST_UPDATED) })}</p>
        ...
        <div className="mt-8 space-y-8 text-sm leading-relaxed text-foreground/90">
          ...
          <Section title={t('二、我們蒐集的資料')}>
            <List
              items={[
                [t('帳號資料'), t('當你使用 Google 登入時，我們會取得你的 Email、姓名與頭像，用於建立並辨識你的帳號。')],
          ...
          <Section title={t('六、你的權利')}>
            <p>
              {t('你有權查詢、更正或刪除你的個人資料。你可以在 App 內刪除自己的紀錄與貼文，或透過下方聯絡方式要求刪除整個帳號與相關資料。')}
            </p>
          </Section>

          <Section title={t('七、兒童隱私')}>
            <p>
              {t('本服務並非針對 13 歲以下兒童設計，我們不會在知情的情況下蒐集兒童的個人資料。')}
            </p>
          </Section>

          <Section title={t('八、政策更新')}>
            <p>
              {t('我們可能會不時更新本政策。重大變更時會於本頁公告，並更新上方的「最後更新」日期。')}
            </p>
          </Section>

          <Section title={t('九、聯絡我們')}>
            <p>
              {t('對本政策或你的資料有任何疑問，歡迎透過以下方式聯絡我們：')}
            </p>
