...
def _rates(model: str) -> dict[str, float]:
    # 呼叫端常帶日期尾碼（如 claude-haiku-4-5-20251001），PRICING 用無日期的
    # 別名當 key。先精準查，找不到再去掉結尾的 -YYYYMMDD 後重查，避免帶日期的
    # 模型一律落到 _DEFAULT（Sonnet 價）導致 Haiku 成本被高估數倍。
    if model in PRICING:
        return PRICING[model]
    base = re.sub(r"-\d{8}$", "", model)
    return PRICING.get(base, _DEFAULT)
